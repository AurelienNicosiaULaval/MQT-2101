...
Dans la quadratique, x et x² varient simultanément. On doit comparer les prédictions aux deux valeurs de x; le coefficient de x, pris seul, n’est pas une pente constante. Ici, les différences quadratiques valent `r nombre_fr(diff(predictions_taux)[1])` puis `r nombre_fr(diff(predictions_taux)[2])` minute.
...
## Exercice 7 - Une courbe de consommation {#exercice-07}
...
## Exercice 8 - Interpréter un logarithme {#exercice-08}
...
## Exercice 9 - Changer d’unité sans changer la courbe {#exercice-09}
...
## Exercice 11 - Repérer une comparaison invalide {#exercice-11}
...
## Exercice 12 - Une bonne moyenne peut cacher un biais {#exercice-12}
...
## Exercice 13 - Choisir le bon intervalle {#exercice-13}
...
À x = 3,5, les deux intervalles sont centrés sur `r nombre_fr(ic_lots[1,"fit"], 3)` heures. L’intervalle pour la moyenne va de `r nombre_fr(ic_lots[1,"lwr"], 3)` à `r nombre_fr(ic_lots[1,"upr"], 3)` heures; celui pour un nouveau lot va de `r nombre_fr(ip_lots[1,"lwr"], 3)` à `r nombre_fr(ip_lots[1,"upr"], 3)` heures. Le second inclut la variabilité d’une nouvelle observation, en plus de l’incertitude d’estimation de la moyenne.
...
## Exercice 14 - Réviser une recommandation {#exercice-14}
