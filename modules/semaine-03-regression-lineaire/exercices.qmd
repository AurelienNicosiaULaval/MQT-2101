...
## Pratique supplémentaire {#pratique-supplementaire}
...
Pour les exercices 9 à 12, chaque corrigé réajuste son modèle à partir de `performance`. Vous pouvez donc les faire séparément après la préparation. Pour les exercices des ouvrages, consultez les [lectures et exercices ciblés](lectures.qmd#exercices-ouvrages).

## Exercice 7 - Prédiction, résidu et unités {#exercice-07}
...
:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 7"}
...
```{r}
#| label: solution-03-cout-livraisons
...
## Exercice 8 - Retrouver la droite à la main {#exercice-08}
...
1. Calculez $\bar{x}$, $\bar{y}$, $S_{xx} = \sum_i (x_i-\bar{x})^2$ et $S_{xy} = \sum_i (x_i-\bar{x})(y_i-\bar{y})$.
2. Utilisez $\hat\beta_1 = S_{xy}/S_{xx}$ et $\hat\beta_0 = \bar{y}-\hat\beta_1\bar{x}$ pour trouver la droite.
...
:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 8"}

Les moyennes sont $\bar{x}=6$ et $\bar{y}=7{,}2$. Les sommes centrées valent $S_{xx}=40$ et $S_{xy}=-20$. On obtient donc $\hat\beta_1=-0{,}5$ et $\hat\beta_0=10{,}2$, soit $\hat{y}=10{,}2-0{,}5x$.
...
```{r}
#| label: solution-03-droite-manuelle
...
## Exercice 9 - Passer des minutes aux secondes {#exercice-09}
...
:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 9"}

Puisque $x_{\mathrm{secondes}}=60x_{\mathrm{minutes}}$, la pente par seconde est la pente par minute divisée par 60. L’ordonnée à l’origine, les valeurs ajustées, les résidus et le R² restent les mêmes, à l’arrondi numérique près. Le changement d’unité ne change pas l’information contenue dans les observations.

```{r}
#| label: solution-03-changement-unites
...
La pente est d’environ `r nombre_fr(coef(modele_secondes)[[2]], 6)` point de satisfaction par seconde. Deux minutes correspondent à 120 secondes; la différence prédite est donc d’environ `r nombre_fr(120 * coef(modele_secondes)[[2]], 3)` point. Il s’agit toujours d’une association entre moyennes mensuelles, pas d’un effet causal établi.

::::

## Exercice 10 - Distinguer corrélation et R² {#exercice-10}
...
:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 10"}

```{r}
#| label: solution-03-correlation-r2
...
La corrélation vaut environ `r nombre_fr(r_service, 3)`; son carré et le R² valent environ `r nombre_fr(r_service^2, 3)`. La relation linéaire observée est négative. Le carré perd le signe : le R² ne donne donc pas la direction de l’association.

Le modèle décrit environ `r nombre_fr(100 * r_service^2, 1)` % de la variabilité observée de la satisfaction autour de sa moyenne. Ce n’est pas un pourcentage de prédictions exactes. Pour étudier les erreurs, on examine notamment les résidus; pour évaluer de nouvelles prédictions, il faut aussi une validation adaptée.
...
## Exercice 11 - Moyenne ou nouvelle observation? {#exercice-11}
...
:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 11"}

```{r}
#| label: solution-03-deux-intervalles
...
La prédiction commune vaut environ `r nombre_fr(ic_moyenne[1, "fit"], 3)` points sur 10. L’intervalle pour la moyenne est [`r nombre_fr(ic_moyenne[1, "lwr"], 3)`; `r nombre_fr(ic_moyenne[1, "upr"], 3)`], contre [`r nombre_fr(ip_observation[1, "lwr"], 3)`; `r nombre_fr(ip_observation[1, "upr"], 3)`] pour une nouvelle observation mensuelle.
...
## Exercice 12 - Repérer l’extrapolation {#exercice-12}
...
:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 12"}

```{r}
#| label: solution-03-extrapolation
...
Les délais observés vont de `r nombre_fr(plage_delais[1], 1)` à `r nombre_fr(plage_delais[2], 1)` minutes. Seule la demande à 5 minutes est une interpolation. Les demandes à 2, 9 et 40 minutes sont des extrapolations, même si 9 minutes est proche de la borne supérieure.
...
## Exercice 13 - Lire trois diagnostics {#exercice-13}
...
```{r}
#| label: enonce-03-diagnostics-fictifs
#| echo: false
#| fig-width: 9
#| fig-height: 3.6
#| fig-alt: "Trois diagnostics fictifs : A montre une courbure, B une dispersion croissante et C des séries temporelles de résidus de même signe."
set.seed(210113)
x_a <- seq(-2, 2, length.out = 60)
y_a <- 10 + 3 * x_a + 1.5 * x_a^2 + rnorm(60, sd = 0.3)
x_b <- rep(1:30, each = 2)
y_b <- 5 + 0.5 * x_b + rep(c(-1, 1), 30) * (0.3 + x_b / 12)
x_c <- sample(1:60)
y_c <- 5 + 0.1 * x_c + rep(c(-1, 1, -1), each = 20) + rnorm(60, sd = 0.15)
modeles_diagnostic <- list(lm(y_a ~ x_a), lm(y_b ~ x_b), lm(y_c ~ x_c))
diagnostics_fictifs <- bind_rows(
  tibble(panneau = "A : valeur ajustée", axe = fitted(modeles_diagnostic[[1]]),
         residu = residuals(modeles_diagnostic[[1]])),
  tibble(panneau = "B : valeur ajustée", axe = fitted(modeles_diagnostic[[2]]),
         residu = residuals(modeles_diagnostic[[2]])),
  tibble(panneau = "C : ordre chronologique", axe = seq_len(60),
         residu = residuals(modeles_diagnostic[[3]]))
)
ggplot(diagnostics_fictifs, aes(x = axe, y = residu)) +
  geom_hline(yintercept = 0, colour = "grey55", linetype = "dashed") +
  geom_point(colour = "#28536b", size = 1.5) +
  facet_wrap(~ panneau, scales = "free", nrow = 1) +
  labs(x = "Axe indiqué dans chaque panneau", y = "Résidu") +
...
:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 13"}
...
## Exercice 14 - Corriger un compte rendu {#exercice-14}
...
1. Testez $H_0:\beta_1=0$ contre $H_1:\beta_1\ne 0$ au seuil de 5 %. Calculez $t$, les degrés de liberté, la valeur p et l’intervalle de confiance à 95 % de la pente.
2. Corrigez les quatre phrases suivantes et proposez une conclusion de trois phrases.

> a. « La valeur p est la probabilité que la pente vraie soit nulle. »
>
> b. « Comme le test n’est pas significatif, le budget n’a aucun lien avec les ventes. »
>
> c. « Le R² indique que 6,6 % des prédictions sont correctes. »
>
> d. « Augmenter le budget de 1 000 $ provoquera exactement 2 400 $ de ventes supplémentaires. »

:::: {.callout-tip collapse="true" title="Solution détaillée de l’exercice 14"}

Pour une régression simple avec constante, les degrés de liberté sont $50-2=48$. La statistique est $t=2{,}4/1{,}3\approx 1{,}846$. On calcule la valeur p bilatérale et l’intervalle classique avec la loi de Student :

```{r}
#| label: solution-03-lecture-critique
pente_exemple <- 2.4
erreur_type <- 1.3
degres_liberte <- 50 - 2
...
La valeur p est d’environ `r nombre_fr(valeur_p, 3)` et l’intervalle va de `r nombre_fr(ic_exemple[1], 3)` à `r nombre_fr(ic_exemple[2], 3)` milliers de dollars de ventes par millier de dollars de budget. Le test ne rejette pas $H_0$ à 5 %; l’intervalle contient zéro, ce qui concorde avec le test bilatéral.
