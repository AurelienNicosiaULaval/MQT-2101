...
Cette page rassemble des exemples pour débloquer un geste précis après une première tentative. La [mission](../../modules/atelier-01-r/guide-atelier.llms.md) fixe les productions à réaliser. Il n’est pas nécessaire de reproduire toutes les démonstrations dans le rapport.
...
> **NOTE:**
>
> Repérez la démonstration qui répond à votre difficulté, puis adaptez son code dans votre fichier Quarto. Vérifiez la sortie et expliquez le résultat avec vos propres mots. Les choix d’analyse et la réponse à la direction vous appartiennent.
...
>     mois_satisfaction = sum(!is.na(satisfaction)),
>     satisfaction_moyenne = mean(satisfaction, na.rm = TRUE),
>     mois_delai = sum(!is.na(delai_livraison_jours)),
...
>     Mois = mois_observes,
>     `Ventes totales ($ CA)` = currency_ca(ventes_totales),
>     `Clients cumulés` = clients_totaux,
>     `Ventes par client ($ CA)` = currency_ca(panier_moyen)
>   ) |>
>   knitr::kable()
> ```
>
> | Succursale | Mois | Ventes totales (\$ CA) | Clients cumulés | Ventes par client (\$ CA) |
> |:---|---:|:---|---:|:---|
> | Montréal | 12 | 1 923 795 \$ | 31351 | 61,36 \$ |
> | Québec | 12 | 1 740 846 \$ | 27586 | 63,11 \$ |
> | Sherbrooke | 12 | 1 453 016 \$ | 23858 | 60,90 \$ |
> | Gatineau | 12 | 1 442 128 \$ | 24166 | 59,68 \$ |
> | Trois-Rivières | 12 | 1 403 919 \$ | 22334 | 62,86 \$ |
>
> ``` r
> # Présenter séparément les indicateurs de service pour garder des tableaux lisibles.
> synthese_succursales |>
>   transmute(
>     Succursale = succursale,
>     `Mois avec satisfaction` = mois_satisfaction,
>     `Satisfaction sur 10` = satisfaction_moyenne,
>     `Mois avec délai` = mois_delai,
>     `Délai (jours)` = delai_moyen,
>     `Ruptures totales` = ruptures_stock
>   ) |>
>   knitr::kable(digits = 2, format.args = list(decimal.mark = ","))
> ```
>
> | Succursale | Mois avec satisfaction | Satisfaction sur 10 | Mois avec délai | Délai (jours) | Ruptures totales |
> |:---|---:|---:|---:|---:|---:|
> | Montréal | 11 | 7,75 | 12 | 2,53 | 25 |
> | Québec | 10 | 7,83 | 12 | 2,75 | 23 |
> | Sherbrooke | 12 | 7,57 | 11 | 2,71 | 28 |
> | Gatineau | 12 | 7,74 | 11 | 2,75 | 28 |
> | Trois-Rivières | 12 | 7,39 | 12 | 2,64 | 29 |
>
> À observer :
>
> - les ventes totales comparent le volume d’affaires;
> - le ratio ventes/clients compare les ventes par client sur la période;
...
>
> Les ventes et les clients sont complets dans ce fichier. Pour la satisfaction et le délai, les effectifs indiquent les mois disponibles. Les moyennes portent sur les résumés mensuels, avec un poids égal pour chaque mois disponible. La mission demande un seul indicateur de service : choisissez celui qui vous est utile parmi les exemples présentés ici.
>
> À reprendre dans la trace finale :
>
...
>
> ``` r
> observations_graphique <- ventes |>
>   filter(!is.na(ruptures_stock), !is.na(satisfaction))
>
> observations_graphique |>
...
> - le nuage permet de repérer des associations et de la dispersion;
> - les couleurs permettent de repérer les succursales, mais elles ne remplacent pas une analyse plus formelle.
>
> Le graphique utilise 57 observations sur 60. Il exclut les lignes dont une des deux variables est manquante.
>
> > **CAUTION:**
> >
